...
public class ArticlesApiIntegrationTests : IClassFixture<CustomWebApplicationFactory>, IAsyncLifetime
{
    ...
    public async Task Create_WithValidToken_Returns201AndArticleIsReadableAfterwards()
    {
        ...
        var createResponse = await SendAuthorizedAsync(HttpMethod.Post, "/api/articles", token, dto);
    ...
    public async Task Update_AsNonOwnerNonAdmin_Returns403()
    {
        ...
        var createDto = new CreateArticleDto(
            ...
            1);
        var createResp = await SendAuthorizedAsync(HttpMethod.Post, "/api/articles", adminToken, createDto);
        ...
        var response = await SendAuthorizedAsync(HttpMethod.Put, $"/api/articles/{created!.Id}", userToken, updateDto);
