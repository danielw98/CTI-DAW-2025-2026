...
public class StudentsController : ControllerBase
{
    ...
    [HttpGet("filter")]
    public IActionResult Filter([FromQuery] decimal? minAverage)
    {
        ...
        {
            if (student.Average >= (double)minAverage)
            {
    ...
    [HttpGet("top")]
    public IActionResult Top([FromQuery] decimal? minAverage)
    {
        if (minAverage == null || minAverage < 1 || minAverage > 10)
        {
            return BadRequest("minAverage must be in range or not null.");
        ...
        for( int i=0; i < topStudents.Count; i++ )
            for(int j=0; j < topStudents.Count; j++)
            {
                if (topStudents[i].Average>topStudents[j].Average)
                {
    ...
    public IActionResult GetStats()
    {
        ...
        foreach(var s in students)
        {
            ...
            if (s.Average < 5)
                allPassing = false;
            if (anyComputerScience && !allPassing)
                break;
        }
